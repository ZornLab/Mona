...
#' @param assay Which assay contains RNA data, usually 'RNA' or 'SCT'. Must include normalized counts in the 'data' slot.
#' @param atac_assay Which assay contains ATAC data, usually 'ATAC'. Must include fragment files and annotations, and 'nCount_ATAC' must be in metadata.
...
#' @param markers Whether to pre-calculate markers. Set true if you use markers often, but processing time can be long for many annotations.
#' @param scores Whether to calculate ranks for gene set scores. Set false if you don't use this feature, as it adds to processing time.
#' @param password A password to open the Mona directory. NOT secure, is stored as plain text in 'mona.qs'. Meant for controlling access to datasets when hosting Mona.
#' @return A 'Mona directory' that can be loaded into Mona
#' @export
save_mona_dir <- function(seurat=NULL,assay=NULL,atac_assay=NULL,counts=NULL,meta=NULL,coords=NULL,dir=NULL,name=NULL,description=NULL,species="human",scores=T,markers=F,password=NULL) {
  mona <- list()
  ...
  if (!is.null(seurat) && !is.null(assay)) {
    ...
    if (!is.null(atac_assay) && class(seurat[[atac_assay]]) == "ChromatinAssay") {
        print("Saving fragment data")
        frag_list <- seurat[[atac_assay]]@fragments
        frag_list <- frag_list[sapply(frag_list,function(x) length(x@cells) > 0)]
        frag_obj_list <- lapply(1:length(frag_list), function(x) {
          open_fragments_10x(frag_list[[x]]@path) %>%
            BPCells::select_cells(frag_list[[x]]@cells) %>% 
            prefix_cell_names(paste0("sample",x,"_")) %>%
        ...
        gtf <- data.frame(chr=chromosomes,
                          ...
                          gene_name=annotations$gene_name) %>% distinct()
        missing <- is.na(gtf$gene_name)
        gtf$gene_name[missing] <- gtf$gene_id[missing]
        gtf[is.na(gtf)] <- ""
        mona[["gtf"]] <- gtf
    }
